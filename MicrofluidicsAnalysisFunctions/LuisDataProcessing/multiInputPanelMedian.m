function out=multiInputPanelMedian(varargin)
...
for i= 2:nargin
   ...
   %SEM
   out.median(i-1,:)=nonzeroColMedian(varargin{i}.cellInf(2).mean(:,range))
   xrange=(1:size(varargin{i}.cellInf(2).mean(:,range), 2))*5/60
   errorbar(xrange, nonzeroColMedian(varargin{i}.cellInf(2).mean(:,range)), nonZeroColStd(varargin{i}.cellInf(2).mean(:,range))/sqrt(size(varargin{i}.cellInf(2).mean, 1)))
   xlim([0, xrange(end)])
   xlabel('Time (Hrs)')
   ylabel('Median cell FL (±SEM)')
   %STD
