function out=multiInputPanelmCherry(varargin)
...
for i= 2:nargin
    ...
    out.cy5(i-1,:)=normalizeTS(nonzeroColMean(varargin{i}.cellInf(2).imBackground(:,range)));
    plot((1:size(varargin{i}.cellInf(2).imBackground(:,range), 2))*5/60, normalizeTS(nonzeroColMean(varargin{i}.cellInf(2).imBackground(:,range))), 'Color', [0 0 0]);
    xlabel('Time (Hrs)');
    ylabel('scaled Fluorescein');
   ...
   %SEM
   out.mean(i-1,:)=nonzeroColMean(varargin{i}.cellInf(3).mean(:,range));
   xrange=(1:size(varargin{i}.cellInf(2).mean(:,range), 2))*5/60;
   errorbar(xrange, nonzeroColMean(varargin{i}.cellInf(3).mean(:,range)), nonZeroColStd(varargin{i}.cellInf(3).mean(:,range))/sqrt(size(varargin{i}.cellInf(3).mean, 1)));
   
   xlim([0, xrange(end)]);
   xlabel('Time (Hrs)');
   ylabel('Mean mCherry FL (±SEM)');
   %STD
   %errorbar((1:size(varargin{i}.cellInf(2).mean(:,range), 2))*5/60, nonzeroColMean(varargin{i}.cellInf(2).mean(:,range)), nonZeroColStd(varargin{i}.cellInf(2).mean(:,range)))
    ...
    plot((1:size(varargin{i}.cellInf(3).mean(:,range), 2))*5/60, zero2NaN(varargin{i}.cellInf(3).mean(:,range))');
    xlabel('Time (Hrs)');
    ylabel('raw cell FL');
    c=c+1;;
end
