...
function experimentReport2(range, multichamber, window,  varargin)
...
for j=1:numel(nms)
  ...
  eval(['cExperiment= multichamber.' nms{j} ';']);
 out=probRateAbovePercentile(range,cExperiment);
 
 plot(range(1:end-1)*5/60, out);
 xlabel('Time');
 ylabel('fraction of cells with rate in top percentile');
 ylim([0,1]);
 hold on;
 title(sprintf(['Probability of cells peaking at any time']));
