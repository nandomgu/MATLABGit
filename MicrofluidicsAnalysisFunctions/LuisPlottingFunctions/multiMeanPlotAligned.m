function h=multiMeanPlotAligned(range, multichamber, cmap)
...
for j= 1:numel(nms)
    ...
    boundedline(aligntoUpshift(range,cExperiment)*5/60, nonzeroColMean(cExperiment.cellInf(2).mean(:,range)), prepareBound(cExperiment.cellInf(2).mean(:,range), @nonZeroColSEM), 'cmap', cmap(j,:))
    xlabel('Time (Hrs)')
    ylabel('Mean cell FL (±SEM)')
    subplot(3,1,3)
    hold on;
    plot(aligntoUpshift(range, cExperiment)*5/60, zero2NaN(cExperiment.cellInf(2).mean(:,range))', ':', 'Color', cmap(j,:))
    xlabel('Time (Hrs)')
