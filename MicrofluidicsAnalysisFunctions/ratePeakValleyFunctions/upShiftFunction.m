function [val, err, arr]=upShiftFunction(range,cExperiment, funcwindow, func )
...
if isempty(range)
    ...
    [cellval, cellerr, indices,upShiftIndex,celltimes,den,ratePeakFigure]=ratePeakTimes(range, cExperiment,3,2, 1);
    ...
    %flmatr=flmatr- nanmean(nanmean(flmatr(:,subwindow))); % we subtract the average of the baseline window positions. 
    flmatr=flmatr-repmat(nanmean(flmatr(:,subwindow),2), 1, size(flmatr,2))
    
    if nargin<3
    
        flmatr=flmatr(:,find(alignedRange==0):(upShiftIndex-5)); %we subtract the window of the matrix that corresponds to after the downshift. 
