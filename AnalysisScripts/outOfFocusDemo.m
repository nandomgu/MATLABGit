...
title('image 109')
    ...
    vecy=sd;
subplot(2,3,2)
scatter(vecx(:), vecy(:)); xlabel('image mean'), ylabel(['image standard deviation'])
...
while j<20
    ...
    if button==1
    ind=find(pdist2([vecx(:) vecy(:)],positiveset(j,:))==min(pdist2([vecx(:) vecy(:)],positiveset(j,:))));
    [I,J]=ind2sub(size(vecy), ind)
    hold on; hp{j}=scatter(vecx(ind),vecy(ind), [],[1,0,0])
    subplot(2,3,6)
...
npoints=10
if drawline==1
    title('Select ' num2str(npoints) ' points to draw a boundary between classes')
...
while j<20
    ...
    hold on; hp{j}=scatter(mn(ind),sd(ind), [],[1,0,1])
    subplot(2,3,6)
    im=d.cExperiment.returnTimelapse(I).returnSingleTimepointRaw(J,6);
    imshow(im, [])
    title('last clicked image')
