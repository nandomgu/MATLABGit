...
cy5(n, :)= normalizeTS(nonzeroColMean(cExperiment.cellInf(bgchan).mean))
end
...
plotcons=[0.2, 0.4, 1]; % just interested in 1% for now
flag=0
  figure;
 mod=1 ;
 ...
 colorchoice=cmap(sCon, :);
   ...
   if ~isempty(conlist)
    ...
    for k=1:numel(conlist)
        ...
        boundedline(times, means(1,:),bnds.(strn), 'cmap', concolors(j, :), 'transparency', 0.1, 'alpha');
       %inputdlg('')
        h=gca;
        a= get(h, 'children');
